...
var LogiaUtils = {
    logToFile: function(){/* No-Op when running in browser */},
...
if(IN_SERVER_MODE){
    ...
    LogiaUtils.logToFile = logToFile = function(logger, buslineId, formattedLogMsg){
        var loggerName = logger.name;
        var appender = logger._getLoggerRegexPropertyValue("appenders");
        if(appender && appender.filepath){
            appender.fullFilePath = FileAppender.getDestFileFullpath(appender.filepath);
            // ** TODO dont pass appender. just pass path
            appender && bus[buslineId].triggerLogToFile(loggerName, formattedLogMsg, appender);
        }
    };
}
