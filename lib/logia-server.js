    ...
    logiaServer.server.on("connection", function (wsConnection) {
        ...
        if(isSlaveClient){
            ...
            wsConnection.on("close", function(message) {
                ...
                logiaServer.numOfSlaves--;
                if(!logiaServer.isInSlaveMode()){
                    bus[logiaServer.eventBusline].trigger("disableSlaveMode");
                }
            });

            bus[logiaServer.eventBusline].trigger("enableSlaveMode", logiaServer.url);
        ...
        wsConnection.on("message", function(logJSON) {
            try {
                ...
                if(logiaServer.isInSlaveMode()){
                    ...
                    logiaServer.config.stdout && bus[logiaServer.eventBusline].triggerStdoutLog(logObj.logMessage.color());
                    var logger = require("./logia-factory.js")(logObj.name)
                    bus[logiaServer.eventBusline].triggerAppendersLog(logger, logObj);
                }
...
LogiaServer.prototype.isInSlaveMode = function (onStop){
    var logiaServer = this;
