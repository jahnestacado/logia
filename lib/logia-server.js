...
LogiaServer.prototype.start = function (){
    ...
    logiaServer.server.on("connection", function (wsConnection) {
        ...
        if(isSlaveClient){
            ...
            bus[logiaServer.buslineId].trigger("enableFullstackMode", logiaServer.url);
        ...
        wsConnection.on("message", function(logJSON) {
            try {
                ...
                if(logiaServer.isInFullstackMode()){
                    logiaServer.config.stdout && bus[logiaServer.buslineId].triggerFullstackStdoutLog(logObj.message, logObj.level);
                    bus[logiaServer.buslineId].triggerFullstackAppendersLog(logObj);
                }
