...
var HermesLog = function (confDirPath) {
	var HermesLog = this;

	HermesLog.confDirPath = confDirPath;
	HermesLog.confFilePath = path.join(confDirPath, CONFIG_FILENAME);
...
HermesLog.prototype.configInit = function () {
	var HermesLog = this;
	var confFilePath = HermesLog.confFilePath;
	...
	if (!fs.existsSync(confFilePath)) {
		mkdirp.sync(HermesLog.confDirPath);
...
HermesLog.prototype.loadConfiguration = function loadConfiguration() {
	var HermesLog = this;
	var fileConf = JSON.parse(fs.readFileSync(HermesLog.confFilePath, "utf8"));
...
HermesLog.prototype.subscribeEventListeners = function () {
	...
	bus.subscribe({
		onLogEvent: function (logObject) {
			...
			config.destFile && IN_SERVER_MODE && HermesLog.logToFile(formattedLogMsg, config.destFile);
...
HermesLog.prototype.setConf = function (options) {
	...
	fs.writeFile(HermesLog.confFilePath, JSON.stringify(config, null, 2), function (error) {
		if (error) {
...
var Constructor = function(confDirPath){
	return new HermesLog(confDirPath);
};
