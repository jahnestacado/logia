...
var UtilityBelt = {
    ...
    centerAlignString: function(str, extendToLength){
        ...
        if(diff > 0){
            extendedStr = " ".repeat(Math.floor(diff/2)) + extendedStr + " ".repeat(Math.ceil(diff/2));
        }
        return extendedStr;
    },
    ...
    updatePrettyLoggerNames: function(name){
        Object.defineProperty(LOGGER_NAMES_PRETTY_PRINT_FORMATTING, name, {});
        Object.keys(LOGGER_NAMES_PRETTY_PRINT_FORMATTING).forEach(function(key){
            LOGGER_NAMES_PRETTY_PRINT_FORMATTING[key] = UtilityBelt.centerAlignString(key, LOGGER_NAME_MAX_LENGTH);
        });
    }
};
...
var Constructor = function(name){
    ...
    if(LOGGERS[name]){
        ...
        LOGGER_NAMES_PRETTY_PRINT_FORMATTING[name] = UtilityBelt.centerAlignString(name, LOGGER_NAME_MAX_LENGTH);
        UtilityBelt.updatePrettyLoggerNames(name);
