...
HermesLog.prototype.getLoggerInstanceProperty  = function(property){
	...
	var loggerName = hermesLog.name;
	var level = GLOBAL_CONFIG[property] && GLOBAL_CONFIG[property][loggerName];

	return level || GLOBAL_CONFIG[property].global;
};
...
HermesLog.prototype.convertToString = function(value){
	...
	if(IN_SERVER_MODE){
		stringValue = typeof value === "object" ? util.inspect(value, false, null) : value.toString();
	} else{
	...
	if(!(bus[name] && bus[name].hasEvent("logEvent"))){
		bus.subscribe(name, {
			onLogEvent: function (logObject) {
				...
				GLOBAL_CONFIG.destFile && IN_SERVER_MODE && fileAppender.logToFile(formattedLogMsg, fileAppender.getDestFileFullPath(GLOBAL_CONFIG.destFile));
