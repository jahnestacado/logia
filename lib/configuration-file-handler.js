...
var ConfigFileHandler = function(){
    ...
    configFileHandler.onConfigurationChangedListener = {
        onConfigurationChanged: function() {
            var oldAppenders = RUNTIME_CONFIG.appenders;
            try {
                configFileHandler.loadConfiguration();
                if(RUNTIME_CONFIG.appenders){
                    Object.keys(RUNTIME_CONFIG.appenders).forEach(function(key){
                        var oldAppender = oldAppenders[key];
                        if (!(oldAppender && oldAppender.filepath && oldAppender.filepath === RUNTIME_CONFIG.appenders[key].filepath)) {
                            FileAppender.setupLogFile(RUNTIME_CONFIG.appenders[key].filepath);
                        }
                    });
                }
            } catch(error){
