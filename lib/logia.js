...
var UtilityBelt = {
    ...
    subscribeBusListeners: function(logger){
        bus.subscribe(BUSLINE_ID, {
            afterConfigurationChanged: function () {
                ...
                var remoteLoggerInstanceConfig = UtilityBelt.getLoggerRegexPropertyValue(logger.name, "remotes");
                logger.remote.terminate && logger.remote.terminate();
    ...
    setupLogServer: function(){
        if(IN_SERVER_MODE){
            var host = GLOBAL_CONFIG.server.interface;
            var port = GLOBAL_CONFIG.server.port;
            var areOptionsValid = host && port;
            var shouldCreateNewServer = logServer ? (logServer.host !== host) || (logServer.port !== port) : true;
            if(logServer && areOptionsValid && shouldCreateNewServer){
                logServer.stop(function(){
                    logServer = new LogServer(GLOBAL_CONFIG, BUSLINE_ID, ON_SERVER_MSG_LISTENERS).start();
                });
...
if(IN_SERVER_MODE){
    ...
    UtilityBelt.logToFile = function(logger, formattedLogMsg){
        ...
        if(appender && appender.filepath){
            ...
            appender && bus[BUSLINE_ID].triggerLogToFile(loggerName, formattedLogMsg, appender);
        }
    };
...
var Logia = function (name) {
    ...
    var remoteLoggerInstanceConfig = UtilityBelt.getLoggerRegexPropertyValue(name, "remotes");
    logger.remote = UtilityBelt.createRemote(remoteLoggerInstanceConfig);
    UtilityBelt.updateActiveLevels(logger);
};
...
Logia.prototype.setAppender = function (appenderOptions) {
    if(IN_SERVER_MODE){
        var logger = this;
        UtilityBelt.setLoggerInstanceProperty(logger.name, "appenders", appenderOptions);
    } else{
...
Logia.prototype.setRemote = function (protocol, url) {
    var logger = this;
    var loggerName = logger.name;
    UtilityBelt.setLoggerInstanceProperty(loggerName, "remotes", {protocol: protocol, url: url});
...
ModuleConstructor.slaveOf = function(targetUrl) {
    var slaveRemote = UtilityBelt.createRemote({protocol: "ws", url: targetUrl, tag: "slave"});
