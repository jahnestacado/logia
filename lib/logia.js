...
Logia.prototype._log = function(logSpecs){
    ...
    GLOBAL_CONFIG.stdout && bus[BUSLINE_ID].triggerDefaultStdoutLog(logObj.message, logObj.level);
    bus[BUSLINE_ID].triggerDefaultAppendersLog(logger, logObj);
    bus[BUSLINE_ID].triggerRemoteLog(logger, logObj);
};
...
Logia.prototype._createLogMessageHeader = function (timestamp, level) {
    ...
    return  timestamp
            ...
            + " [" + loggerName + "]" + LogiaUtils.repeat(" ", ActiveLoggerNameLengthHandler.getMax() - loggerName.length);
};
