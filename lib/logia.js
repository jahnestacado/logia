...
var Logia = function (name) {
    ...
    var remoteLoggerInstanceConfig = logger._getLoggerRegexPropertyValue("remotes");
    logger.remote = LogiaUtils.createRemote(remoteLoggerInstanceConfig);
...
Logia.prototype._getLoggerRegexPropertyValue = function(property){
...
Logia.prototype._updateActiveLevels = function () {
    var logger = this;
    var level = logger._getLoggerRegexPropertyValue("level");
    var activeLevels = [];
    switch (level) {
        case "trace" :
...
Logia.prototype._log = function(logObject){
    var logger = this;
    var formattedLogMsg = logger._createLogMessageHeader(logObject) + " : " + LogiaUtils.getParameterizedMessage(logObject);
    GLOBAL_CONFIG.stdout && console.log(formattedLogMsg[LEVELS[logObject.level].color]);

    LogiaUtils.logToFile(logger, BUSLINE_ID, formattedLogMsg);
...
Logia.prototype._createLogMessageHeader = function (logObject) {
    var loggerName = this.name;
...
Logia.prototype.setRemote = function (protocol, url) {
    ...
    logger.remote = LogiaUtils.createRemote(GLOBAL_CONFIG.remotes[loggerName]);
};
