...
Logia.prototype._log = function(logSpecs){
    ...
    var logObj = {
        ...
        message: logger._createLogMessageHeader(timestamp, logSpecs.level)
                + " : "
                + colors.cyan(Utils.getParameterizedMessage(logSpecs)),
    };

    RUNTIME_CONFIG.stdout && bus[EVENT_BUSLINE].triggerDefaultStdoutLog(logObj.message);
...
Logia.prototype._createLogMessageHeader = function (timestamp, level) {
    ...
    return  timestamp
            ...
            + " [" + loggerName + "]"
            + Utils.repeat(" ", ActiveLoggerNameLengthHandler.getMax() - loggerName.length);
};
